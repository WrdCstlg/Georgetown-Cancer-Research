...
Dev/CI runs on SQLite (embedded). Production substrate is pending D4
(PostgreSQL + pgvector proposed, not decided); the schema is portable modulo
the deltas noted in `schema/schema.sql`.
